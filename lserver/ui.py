...
def print_dashboard(nodes, is_running_func):
    ...
    print_box_line(f"  {GOLD}lserver -a <nodo>{RESET}    {ORANGE}Alternar Auto-Heal (Watchdog).{RESET}", 59)
    print_box_line(f"  {GOLD}lserver -b <nodo>{RESET}    {ORANGE}Opciones de Backup (Manual/Automático).{RESET}", 66)
    print_box_line(f"  {GOLD}lserver daemon start{RESET} {ORANGE}Arranca el vigilante en background.{RESET}", 62)
    print_box_line(f"  {GOLD}lserver daemon stop{RESET}  {ORANGE}Detiene el vigilante.{RESET}", 50)
